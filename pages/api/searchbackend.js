...
async function handler(req, res) {
    ...
    if (searchterm != '') {
        var data = await music.find({ $or: [{ "author": { '$regex': `${searchterm}`, '$options': 'i' } }, { "title": { '$regex': `${searchterm}`, '$options': 'i' } }, { "description": { '$regex': `${searchterm}`, '$options': 'i' } }, { "type": { '$regex': `${searchterm}`, '$options': 'i' } }] })
